...
// ReadStructuredCallback is a function that will be called with each Article object that is read from a batch or snapshot.
// You can return a custom error to stop the reading.
type ReadStructuredCallback func(art *StructuredContent) error
...
// StructuredSnapshotsGetter is an interface for getting multiple snapshots.
type StructuredSnapshotsGetter interface {
	GetStructuredSnapshots(ctx context.Context, req *Request) ([]*Snapshot, error)
}

// StructuredSnapshotGetter is an interface for getting a single snapshot by ID.
type StructuredSnapshotGetter interface {
	GetStructuredSnapshot(ctx context.Context, idr string, req *Request) (*Snapshot, error)
}

// StructuredSnapshotHeader is an interface for getting the headers of a single snapshot by ID.
type StructuredSnapshotHeader interface {
	HeadStructuredSnapshot(ctx context.Context, idr string) (*Headers, error)
}

// StructuredSnapshotDownloader is an interface for downloading a single snapshot by ID to a writer.
...
type StructuredSnapshotReader interface {
	ReadStructuredSnapshot(ctx context.Context, idr string, cbk ReadStructuredCallback) error
}
...
func (c *Client) GetStructuredSnapshots(ctx context.Context, req *Request) ([]*Snapshot, error) {
	sps := []*Snapshot{}
...
func (c *Client) GetStructuredSnapshot(ctx context.Context, idr string, req *Request) (*Snapshot, error) {
	snp := new(Snapshot)
