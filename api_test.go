	...
	for _, tcs := range []*downloadStructuredSnapshotTestSuite{
		{
			baseEntityTestSuite: baseEntityTestSuite{
				sts: http.StatusOK,
				fph: "testdata/enwiki_namespace_0.tar.gz",
			},
		},
		{
			baseEntityTestSuite: baseEntityTestSuite{
...
func (s *readSnapshotTestSuite) TestReadStructuredSnapshot() {
	nmc := 0
	err := s.clt.ReadStructuredSnapshot(s.ctx, s.idr, func(art *api.StructuredContent) error {
		s.NotEmpty(art.Name)
		s.NotEmpty(art.Identifier)
		nmc++
		return nil
